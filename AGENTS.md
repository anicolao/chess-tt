...
6.  **Continuous Prompt Recording**: You MUST record ALL user prompts and instructions verbatim on the Pull Request. Initial prompts go in the description; subsequent prompts must be added as PR comments **BEFORE** every push to the PR so that the instruction appears in the PR timeline before the resulting commits.
